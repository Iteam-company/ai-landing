...
**i18n:** the site is localized (English only for now) and has **no root
`app/layout.tsx`**.
...
rest — all prerendered via `generateStaticParams`, because a static export has no
proxy and cannot redirect. `components/lang-switcher.tsx` renders nothing while
`LOCALES` has a single entry. Content is split contract-from-copy: `content/types.ts`
