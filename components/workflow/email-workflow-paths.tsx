...
function useEmailPathMetrics() {
  ...
  const stage4Ref = React.useRef<HTMLDivElement>(null);
  const [metrics, setMetrics] = React.useState<PathMetrics | null>(null);

  React.useLayoutEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    const stageEls = [stage1Ref.current, stage2Ref.current, stage3Ref.current, stage4Ref.current];
  ...
  return { containerRef, stage1Ref, stage2Ref, stage3Ref, stage4Ref, metrics };
}

export interface EmailConnectorPhase {
  signal: boolean;
  fired: boolean;
}

export function EmailWorkflowPaths({
  metrics,
  connectors,
  reduce,
}: {
  metrics: PathMetrics | null;
  connectors: [EmailConnectorPhase, EmailConnectorPhase, EmailConnectorPhase];
  reduce: boolean | null;
}) {
